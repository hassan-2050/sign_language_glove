...
#else
Serial.println(result.classification[1].value);// returns 0.00
Serial.println(result.classification[0].value);//returns 1.00

  // a small value

if (fabs(result.classification[0].value - 1.00) < epsilon){
    myDFPlayer.play(2);
    delay(1000);
}
else if(fabs(result.classification[1].value - 1.00) < epsilon){
    myDFPlayer.play(1);
    delay(1000);
}
